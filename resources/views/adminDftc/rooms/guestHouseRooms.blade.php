                                ...
                                @if(session()->has('loggedInCustomer') || session()->has('loggedInSuperAdmin') || session()->has('loggedInAdminDftc'))
                                    @if($room->room_status != 'Unavailable' && $room->room_status != 'On-Renovation')
                                        <div class="d-flex justify-content-end">
                                            <button onclick="viewGuestHousePreBookingsAdminDftc('{{ $room->id }}')" class="mt-[1 rem] btn lg:rounded-full md:rounded bg-light-green text-white Montserrat hover:bg-dark-green transition ease-in-out duration-500 mr-2 flex items-center space-x-1">
                                            ...
                                            <button type="button" id="guestHouse-booking"
                                                onclick="bookGuestHouse('{{ addslashes(json_encode($room)) }}', '{{ addslashes(json_encode(session('loggedInAdminDftc'))) }}')"
