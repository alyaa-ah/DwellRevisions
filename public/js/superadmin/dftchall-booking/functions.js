function bookDftcHall(room, user) {
    ...
    form.method = 'POST';
    form.action = '/superAdmin/view-dftc-hall-form';
...
$(document).ready(function() {
    ...
    $('#checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc').on('change', computeDaysAndNights);
    $('#checkInDateHallDftc').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#checkOutDateHallDftc').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function computeTotalAmount() {
        ...
        for (var i = 0; i < numberOfDays; i++) {
            ...
            if (isHolidayOrWeekend(currentDay)) {
                numberOfHolidays++;
    ...
    $('#rateHallDftc, #capacityHallDftc, #checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc, #hasLetterHallDftc').on('change', computeTotalAmount);
    $('input[name="hasLetterHallDftc"]').on('change', computeTotalAmount);
    $(document).on('click', '#submitButtonDftcHall', function(event){
        ...
        $.ajax({
            ...
            url: '/superAdmin/booking-dftcHall',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            beforeSend: function() {
                $('#submitButtonDFTCHall').attr('disabled', true);
                Swal.fire({
                    title: 'Loading...',
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomData2(roomNumber);
        });
    }
});
function fetchRoomData2(roomNumber) {

    $.ajax({
        url: '/superAdmin/DFTChall-get-room-data',
        type: 'GET',
        data: {
