function bookStaffHouse(room, user) {
    ...
    form.method = 'POST';
    form.action = '/client/view-staffhouse-pre-reservation-form';
...
$(document).ready(function() {
    ...
    function computeTotalAmount() {
        ...
        var rent = parseInt($('#rentStaffHouse').val());
    ...
    $(document).on('click', '#submitButtonStaffHouse', function(event){
        event.preventDefault();
        ...
        for (let input of maleGuestsInputs) {
            if (!input.value.trim()) {
                $('#staffHouseTerms').modal('hide');
                $('#error-messageStaffHouse').html("<strong>Validation Error!</strong> <br><br> Please input name of male guest!").show();
                $('#submitButton1').attr('disabled', false);
                setTimeout(function () {
                    $('#error-messageStaffHouse').fadeOut('slow', function () {
                        $(this).hide();
        ...
        for (let input of femaleGuestsInputs) {
            if (!input.value.trim()) {
                $('#staffHouseTerms').modal('hide');
                $('#error-messageStaffHouse').html("<strong>Validation Error!</strong> <br><br> Please input name of female guest!").show();
                $('#submitButton1').attr('disabled', false);
                setTimeout(function () {
                    $('#error-messageStaffHouse').fadeOut('slow', function () {
                        $(this).hide();
        ...
        $.ajax({
            ...
            url: '/clientBookingStaffHouse',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            beforeSend: function() {
                $('#submitButtonStaffHouse').attr('disabled', true);
                Swal.fire({
                    title: 'Loading...',
            ...
            success: function(response){
                if(response == 0){
                    ...
                    icon: "success",
                    title: "All set!",
                    text: "Your staff house pre-reservation is now on pending review.",
                    showConfirmButton: true,
                    }).then(function(){
                        $('#submitButtonStaffHouse').attr('disabled', false);
                        window.location = "/client/view-staffhouse-prereservations";
                    });
                }
            },
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomData1(roomNumber);
        });
    }
});
function fetchRoomData1(roomNumber) {

    $.ajax({
        url: '/get-room-dataStaffHouse',
        type: 'GET',
        data: {
