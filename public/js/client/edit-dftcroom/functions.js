function viewClientDftcRoomBooking(booking){
...
function editDftcRoomBooking(booking){
    var data = JSON.parse(booking);
...
function cancelDftcBooking(booking){
    var data = JSON.parse(booking);
...
$(document).ready(function() {
    ...
    $('#editCheckInDateDftcRoom').on('change', function() {
        ...
        if ($(this).val()) {
            $('#editCheckOutDateDftcRoom').prop('disabled', false);  // Enable check-out field if check-in date is set
            var minCheckOutDate = new Date(selectedDate.getTime());
            $('#editCheckOutDateDftcRoom').attr('min', minCheckOutDate.toISOString().split('T')[0]);
        }
        var minCheckOutDate = new Date(selectedDate.getTime());
    ...
    $('#editCheckInDateDftcRoom, #editCheckOutDateDftcRoom, #editArrivalDftcRoom, #editDepartureDftcRoom').on('change', computeDaysAndNightsEditDftcRoom);
    function computeTotalAmountEditDftcRoomClient() {
    ...
    $('#editRateDftcRoom, #editCapacityDftcRoom, #editNumOfMaleDftcRoom, #editNumOfFemaleDftcRoom, #editCheckInDateDftcRoom, #editCheckOutDateDftcRoom, #editHasLetterDftcRoom').on('change', computeTotalAmountEditDftcRoomClient);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditDftcRoomClient(roomNumber);

        });
    }
});
function fetchRoomDataEditDftcRoomClient(roomNumber) {
    $.ajax({
        url: '/get-room-dataEditDftcRoomClient',
        type: 'GET',
        data: {
