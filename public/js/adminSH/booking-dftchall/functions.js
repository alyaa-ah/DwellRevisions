function bookDftcHall(room, user) {
    ...
    form.method = 'POST';
    form.action = '/adminSH/view-DFTC-hall-pre-reservation-form';
...
$(document).ready(function() {
    function getCurrentPhilippineTimeDftcHallAdminSH() {
        var now = new Date();
    ...
    var today = getCurrentPhilippineTimeDftcHallAdminSH();
    ...
    $('#checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc').on('change', computeDaysAndNightsDftcAdminSH);
    $('#checkInDateHallDftc').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#checkOutDateHallDftc').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function computeDaysAndNightsDftcAdminSH() {
    ...
    $('#checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc').on('change', computeDaysAndNightsDftcAdminSH);
    function isHolidayOrWeekendAdminSH(date) {
        var holidays = [
    ...
    function computeTotalAmountDftcHallAdminSH() {
        ...
        for (var i = 0; i < numberOfDays; i++) {
            ...
            if (isHolidayOrWeekendAdminSH(currentDay)) {
                numberOfHolidays++;
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataDftcHallAdminSH(roomNumber);
        });
    }
});
function fetchRoomDataDftcHallAdminSH(roomNumber) {

    $.ajax({
        url: '/get-room-dataDftcHallAdminSH',
        type: 'GET',
        data: {
