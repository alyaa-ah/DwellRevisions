function viewGuestHouseBookingAdminSH(booking){
...
function editGuestHouseBookingAdminSH(booking){
    var data = JSON.parse(booking);
...
function cancelGuestHouseBookingAdminSH(booking){
    var data = JSON.parse(booking);
    $('#GH_bookingID').val(data.id);
    $('#cancel-guesthousebooking-modal').modal('show');
}
$(document).ready(function() {
    function editgetCurrentPhilippineTimeAdminSH() {
        var now = new Date();
    ...
    var today = editgetCurrentPhilippineTimeAdminSH();
    ...
    $('#editCheckInDateGuestHouse, #editCheckOutDateGuestHouse, #editArrivalGuestHouse, #editDepartureGuestHouse').on('change', editcomputeDaysAndNightsAdminSH);
    $('#editCheckInDateGuestHouse').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateGuestHouse').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function editcomputeDaysAndNightsAdminSH() {
    ...
    $('#editCheckInDateGuestHouse, #editCheckOutDateGuestHouse, #editArrivalGuestHouse, #editDepartureGuestHouse').on('change', editcomputeDaysAndNightsAdminSH);
    function editcomputeTotalAmountAdminSH() {
        ...
        if (isNaN(rate) || isNaN(capacity) || isNaN(numOfMale) || isNaN(numOfFemale)) {
            $('#editTotalAmountGuestHouse').val('NaN');
            return;
        }
    ...
    $('#editRateGuestHouse, #editCapacityGuestHouse, #editNumOfMaleGuestHouse, #editNumOfFemaleGuestHouse, #editRentGuestHouse, #editBeddingGuestHouse, #editCheckInDateGuestHouse, #editCheckOutDateGuestHouse, #edithasLetter').on('change', editcomputeTotalAmountAdminSH);
    $('input[name="hasLetter"]').on('change', editcomputeTotalAmountAdminSH);
    $(document).on('click', '#submitButtonEditGuestHouse', function(event){
        ...
        $.ajax({
            ...
            url: '/adminSHEditBookingGuestHouse',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).on('submit', '#cancel-guesthouse-form', function(event){
        ...
        $.ajax({
            ...
            url: '/AdminSHCancelBookingGuestHouse',
            method: 'POST',
            data: $(this).serialize(),
            beforeSend: function() {
                Swal.fire({
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditGuestHouseAdminSH(roomNumber);
        });
    }
});

function fetchRoomDataEditGuestHouseAdminSH(roomNumber) {
    $.ajax({
        url: '/get-room-data-editAdminSH',
        type: 'GET',
        data: {
