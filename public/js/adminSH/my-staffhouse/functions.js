function viewStaffHouseBookingAdminSH(booking){
...
function editStaffHouseBookingAdminSH(booking){
    var data = JSON.parse(booking);
...
function cancelStaffHouseBookingAdminSH(booking){
    var data = JSON.parse(booking);
...
$(document).ready(function() {
    ...
    $('#editCheckInDateStaffHouse, #editCheckOutDateStaffHouse, #editArrivalStaffHouse, #editDepartureStaffHouse').on('change', editComputeDaysAndNightsAdminSH);
    $('#editCheckInDateStaffHouse').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateStaffHouse').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    $('#editRateStaffHouse, #editCapacityStaffHouse, #editNumOfMaleStaffHouse, #editNumOfFemaleStaffHouse, #editBeddingStaffHouse, #editCheckInDateStaffHouse, #editCheckOutDateStaffHouse, #editHasLetterStaffHouse').on('change', editComputeTotalAmountAdminSH);
    $('input[name="hasLetterStaffHouseEdit"]').on('change', editComputeTotalAmountAdminSH);
    $(document).on('click', '#submitButtonEditStaffHouse', function(event){
        ...
        $.ajax({
            ...
            url: '/adminSHEditBookingStaffHouse',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).on('submit', '#cancel-staffhouse-form', function(event){
        ...
        $.ajax({
            ...
            url: '/adminSHCancelBookingStaffHouse',
            method: 'POST',
            data: $(this).serialize(),
            success: function(response){
                if(response == 0){
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditStaffHouseAdminSH(roomNumber);
        });
    }
});

function fetchRoomDataEditStaffHouseAdminSH(roomNumber) {
    $.ajax({
        url: '/get-room-data-editStaffHouseAdminSH',
        type: 'GET',
        data: {
