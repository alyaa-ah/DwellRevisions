function viewAdminSHDftcHallBooking(booking){
...
function editAdminSHDftcHallBooking(booking){
    var data = JSON.parse(booking);
...
$(document).ready(function() {
    function getCurrentPhilippineTimeEditDftcHallAdminSH() {
        var now = new Date();
    ...
    var today = getCurrentPhilippineTimeEditDftcHallAdminSH();
    ...
    $('#editCheckInDateDftcHall, #editCheckOutDateDftcHall, #editArrivalDftcHall, #editDepartureDftcHall').on('change', computeDaysAndNightsEditDftcHallAdminSH);
    $('#editCheckInDateDftcHall').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateDftcHall').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function computeDaysAndNightsEditDftcHallAdminSH() {
    ...
    $('#editCheckInDateDftcHall, #editCheckOutDateDftcHall, #editArrivalDftcHall, #editDepartureDftcHall').on('change', computeDaysAndNightsEditDftcHallAdminSH);
    function isHolidayOrWeekendEditDftcHallAdminSH(date) {
        var holidays = [
    ...
    function computeTotalAmountEditDftcHallAdminSH() {
        ...
        for (var i = 0; i < numberOfDays; i++) {
            ...
            if (isHolidayOrWeekendEditDftcHallAdminSH(currentDay)) {
                numberOfHolidays++;
    ...
    $('#editRateDftcHall, #editCapacityDftcHall, #editCheckInDateDftcHall, #editCheckOutDateDftcHall, #editArrivalDftcHall, #editDepartureDftcHall, #editHasLetterDftcHall').on('change', computeTotalAmountEditDftcHallAdminSH);
    $('input[name="hasLetterDftcHallEdit"]').on('change', computeTotalAmountEditDftcHallAdminSH);
    $(document).on('click', '#submitButtonEditDftcHall', function(event){
        ...
        $.ajax({
            ...
            url: '/adminSHEditBookingDftcHall',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditDftcHallAdminSH(roomNumber);
        });
    }
});
function fetchRoomDataEditDftcHallAdminSH(roomNumber) {

    $.ajax({
        url: '/get-room-dataEditDftcHallAdminSH',
        type: 'GET',
        data: {
