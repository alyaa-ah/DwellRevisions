function viewAdminSHDftcRoomBooking(booking){
...
function editAdminSHDftcRoomBooking(booking){
    var data = JSON.parse(booking);
...
function cancelAdminSHDftcBooking(booking){
    var data = JSON.parse(booking);
    $('#DFTC_bookingID').val(data.id);
    $('#cancel-dftcbooking-modal').modal('show');
}
$(document).ready(function() {
    function getCurrentPhilippineTimeEditDfctRoomAdminSH() {
        var now = new Date();
    ...
    var today = getCurrentPhilippineTimeEditDfctRoomAdminSH();
    ...
    $('#editCheckInDateDftcRoom, #editCheckOutDateDftcRoom, #editArrivalDftcRoom, #editDepartureDftcRoom').on('change', computeDaysAndNightsEditDfctRoomAdminSH);
    $('#editCheckInDateDftcRoom').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateDftcRoom').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function computeDaysAndNightsEditDfctRoomAdminSH() {
    ...
    $('#editCheckInDateDftcRoom, #editCheckOutDateDftcRoom, #editArrivalDftcRoom, #editDepartureDftcRoom').on('change', computeDaysAndNightsEditDfctRoomAdminSH);
    function computeTotalAmountEditDftcRoomAdminSH() {
    ...
    $('#editRateDftcRoom, #editCapacityDftcRoom, #editNumOfMaleDftcRoom, #editNumOfFemaleDftcRoom, #editCheckInDateDftcRoom, #editCheckOutDateDftcRoom, #editHasLetterDftcRoom').on('change', computeTotalAmountEditDftcRoomAdminSH);
    $('input[name="hasLetterDftcRoomEdit"]').on('change', computeTotalAmountEditDftcRoomAdminSH);
    $(document).on('click', '#submitButtonEditDftcRoom', function(event){
        ...
        $.ajax({
            ...
            url: '/adminSHEditBookingDftcRoom',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditDftcRoomAdminSH(roomNumber);

        });
    }
});
function fetchRoomDataEditDftcRoomAdminSH(roomNumber) {
    $.ajax({
        url: '/get-room-dataEditDftcRoomAdminSH',
        type: 'GET',
        data: {
