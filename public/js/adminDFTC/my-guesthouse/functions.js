function viewGuestHouseBookingAdminDftc(booking){
...
function editGuestHouseBookingAdminDftc(booking){
    var data = JSON.parse(booking);
...
function cancelGuestHouseBookingAdminDftc(booking){
    var data = JSON.parse(booking);
...
$(document).ready(function() {
    ...
    $('#editCheckInDateGuestHouse, #editCheckOutDateGuestHouse, #editArrivalGuestHouse, #editDepartureGuestHouse').on('change', editcomputeDaysAndNightsAdminDftc);
    $('#editCheckInDateGuestHouse').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateGuestHouse').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    $('#editRateGuestHouse, #editCapacityGuestHouse, #editNumOfMaleGuestHouse, #editNumOfFemaleGuestHouse, #editRentGuestHouse, #editBeddingGuestHouse, #editCheckInDateGuestHouse, #editCheckOutDateGuestHouse, #edithasLetter').on('change', editcomputeTotalAmountAdminDftc);
    $('input[name="hasLetter"]').on('change', editcomputeTotalAmountAdminDftc);
    $(document).on('click', '#submitButtonEditGuestHouse', function(event){
        ...
        $.ajax({
            ...
            url: '/adminDftcEditBookingGuestHouse',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).on('submit', '#cancel-guesthouse-form', function(event){
        ...
        $.ajax({
            ...
            url: '/AdminDftcCancelBookingGuestHouse',
            method: 'POST',
            data: $(this).serialize(),
            beforeSend: function() {
                Swal.fire({
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditGuestHouseAdminDftc(roomNumber);
        });
    }
});

function fetchRoomDataEditGuestHouseAdminDftc(roomNumber) {
    $.ajax({
        url: '/get-room-data-editAdminDftc',
        type: 'GET',
        data: {
