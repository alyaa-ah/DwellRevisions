function viewStaffHouseBookingAdminDftc(booking){
...
function editStaffHouseBookingAdminDftc(booking){
    var data = JSON.parse(booking);
...
function cancelStaffHouseBookingAdminDftc(booking){
    var data = JSON.parse(booking);
    $('#SH_bookingID').val(data.id);
    $('#cancel-staffhousebooking-modal').modal('show');
}
$(document).ready(function() {
    function editGetCurrentPhilippineTimeAdminDftc() {
        var now = new Date();
    ...
    var today = editGetCurrentPhilippineTimeAdminDftc();
    ...
    $('#editCheckInDateStaffHouse, #editCheckOutDateStaffHouse, #editArrivalStaffHouse, #editDepartureStaffHouse').on('change', editComputeDaysAndNightsAdminDftc);
    $('#editCheckInDateStaffHouse').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateStaffHouse').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function editComputeDaysAndNightsAdminDftc() {
    ...
    $('#editCheckInDateStaffHouse, #editCheckOutDateStaffHouse, #editArrivalStaffHouse, #editDepartureStaffHouse').on('change', editComputeDaysAndNightsAdminDftc);
    function editComputeTotalAmountAdminDftc() {
    ...
    $('#editRateStaffHouse, #editCapacityStaffHouse, #editNumOfMaleStaffHouse, #editNumOfFemaleStaffHouse, #editBeddingStaffHouse, #editCheckInDateStaffHouse, #editCheckOutDateStaffHouse, #editHasLetterStaffHouse').on('change', editComputeTotalAmountAdminDftc);
    $('input[name="hasLetterStaffHouseEdit"]').on('change', editComputeTotalAmountAdminDftc);
    $(document).on('click', '#submitButtonEditStaffHouse', function(event){
        ...
        $.ajax({
            ...
            url: '/adminDftcEditBookingStaffHouse',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).on('submit', '#cancel-staffhouse-form', function(event){
        ...
        $.ajax({
            ...
            url: '/adminDftcCancelBookingStaffHouse',
            method: 'POST',
            data: $(this).serialize(),
            beforeSend: function() {
                Swal.fire({
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditStaffHouseAdminDftc(roomNumber);
        });
    }
});

function fetchRoomDataEditStaffHouseAdminDftc(roomNumber) {
    $.ajax({
        url: '/get-room-data-editStaffHouseAdminDftc',
        type: 'GET',
        data: {
