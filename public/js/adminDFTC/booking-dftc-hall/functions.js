function bookDftcHall(room, user) {
    ...
    form.method = 'POST';
    form.action = '/adminDFTC/view-DFTC-hall-pre-reservation-form';
...
$(document).ready(function() {
    function getCurrentPhilippineTimeDftcHallAdminDftc() {
        var now = new Date();
    ...
    var today = getCurrentPhilippineTimeDftcHallAdminDftc();
    ...
    $('#checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc').on('change', computeDaysAndNightsDftcAdminDftc);
    $('#checkInDateHallDftc').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#checkOutDateHallDftc').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function computeDaysAndNightsDftcAdminDftc() {
    ...
    $('#checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc').on('change', computeDaysAndNightsDftcAdminDftc);
    function isHolidayOrWeekendAdminDftc(date) {
        var holidays = [
    ...
    function computeTotalAmountDftcHallAdminDftc() {
        ...
        for (var i = 0; i < numberOfDays; i++) {
            ...
            if (isHolidayOrWeekendAdminDftc(currentDay)) {
                numberOfHolidays++;
    ...
    $('#rateHallDftc, #capacityHallDftc, #checkInDateHallDftc, #checkOutDateHallDftc, #arrivalHallDftc, #departureHallDftc, #hasLetterHallDftc').on('change', computeTotalAmountDftcHallAdminDftc);
    $('input[name="hasLetterHallDftc"]').on('change', computeTotalAmountDftcHallAdminDftc);
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataDftcHallAdminDftc(roomNumber);
        });
    }
});
function fetchRoomDataDftcHallAdminDftc(roomNumber) {

    $.ajax({
        url: '/get-room-dataDftcHallAdminDftc',
        type: 'GET',
        data: {
