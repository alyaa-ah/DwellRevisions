function viewAdminDftcDftcHallBooking(booking){
...
function editAdminDftcDftcHallBooking(booking){
    var data = JSON.parse(booking);
...
$(document).ready(function() {
    function getCurrentPhilippineTimeEditDftcHallAdminDftc() {
        var now = new Date();
    ...
    var today = getCurrentPhilippineTimeEditDftcHallAdminDftc();
    ...
    $('#editCheckInDateDftcHall, #editCheckOutDateDftcHall, #editArrivalDftcHall, #editDepartureDftcHall').on('change', computeDaysAndNightsEditDftcHallAdminDftc);
    $('#editCheckInDateDftcHall').on('change', function() {
        var selectedDate = new Date($(this).val());
        var minCheckOutDate = new Date(selectedDate.getTime());
        $('#editCheckOutDateDftcHall').attr('min', minCheckOutDate.toISOString().split('T')[0]);
    ...
    function computeDaysAndNightsEditDftcHallAdminDftc() {
    ...
    $('#editCheckInDateDftcHall, #editCheckOutDateDftcHall, #editArrivalDftcHall, #editDepartureDftcHall').on('change', computeDaysAndNightsEditDftcHallAdminDftc);
    function isHolidayOrWeekendEditDftcHallAdminDftc(date) {
        var holidays = [
    ...
    function computeTotalAmountEditDftcHallAdminDftc() {
        ...
        for (var i = 0; i < numberOfDays; i++) {
            ...
            if (isHolidayOrWeekendEditDftcHallAdminDftc(currentDay)) {
                numberOfHolidays++;
    ...
    $(document).on('click', '#submitButtonEditDftcHall', function(event){
        ...
        $.ajax({
            ...
            url: '/adminDftcEditBookingDftcHall',
            method: 'POST',
            data: formData,
            contentType: false,
            processData: false,
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    if (roomNumberElement) {
        roomNumberElement.addEventListener("change", function() {
            ...
            fetchRoomDataEditDftcHallAdminDftc(roomNumber);
        });
    }
});
function fetchRoomDataEditDftcHallAdminDftc(roomNumber) {

    $.ajax({
        url: '/get-room-dataEditDftcHallAdminDftc',
        type: 'GET',
        data: {
